...
async def sigh_in(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите почту✉️ и пароль🔑 через пробел", reply_markup=ReplyKeyboardRemove()
    )
...
async def sigh_up(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите почту✉️, имя🪪 и пароль🔑 через пробелы в соответствующем порядке"
    )
...
async def change_profile(message: types.Message, state: FSMContext):
    await message.answer(
        text="Какие данные хотите изменить?", reply_markup=update_profile_keyboard
    )
...
async def request_change_profile(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✔️ {profile_update_type}: {message.text} - изменения прошли успешно!",
...
async def get_item_by_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        result = await message.answer_photo(
            image_from_pc, caption=item_text, reply_markup=item_id_keyboard
        )
...
async def change_item_by_id(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        text=f"Залог составляет 500 руб.\nВведите планируемую дату возврата в формате dd.mm.yyyy📆",
...
async def change_item_by_id(message: types.Message, state: FSMContext):
    ...
    item_id = int(data.get("current_item_id"))
    await message.answer(
        text="Введите через пробел стоимость залога💰 и планируемую дату возврата в формате yyyy.mm.dd📆",
        reply_markup=ReplyKeyboardRemove(),
    )
    await state.update_data(current_item_id=item_id)
...
async def change_item_by_id(message: types.Message, state: FSMContext):
    await message.answer(
        text="Какие данные хотите изменить?", reply_markup=update_item_keyboard
    )
...
@dp.message(
    F.text.in_({"Название", "Инвентарный номер", "Тип", "Состояние", "Цена"}),
    FSM.change_item_id,
)
async def change_item_by_id_input(message: types.Message, state: FSMContext):
    await message.answer(
        f"⌨️ Введите {message.text}:", reply_markup=ReplyKeyboardRemove()
    )
        ...
        async with session.get(file_url) as resp:
            if resp.status == 200:
                ...
                try:
                    ...
                    await message.reply(
                        "✔️ Фото обновлено!✔", reply_markup=main_keyboard
                    )
...
async def request_change_item_by_id(message: types.Message, state: FSMContext):
    ...
    print(response.json())
    await message.answer(
        f"✔️ {item_update_type}: {message.text} - изменения прошли успешно!",
        reply_markup=main_keyboard,
    )
    await state.set_state(FSM.main_menu_next)
...
async def request_item_create(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✔️ Снаряжение {name} было добавлено", reply_markup=main_keyboard
    )
    await state.set_state(FSM.main_menu_next)
...
async def get_transaction_by_id(message: types.Message, state: FSMContext):
    ...
    if response.status_code == HTTPStatus.OK:
        print(response.json())
        await message.answer(
            "✔️ Транзация завершена успешно", reply_markup=main_keyboard
        )
        await state.set_state(FSM.main_menu_next)
    else:
        print(response.json())
